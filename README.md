...
侧栏里一个面板（v1.64 起不再是弹层）：评分曲线、失着手数、当前这一手的解释，点任意一处跳到那一手。
「局势波动」（只有静态分差）默认折叠。可导出带评注的 SGF，也可以就地推演、重下这一手。

![复盘：侧栏面板里的曲线、失着与解释](docs/screenshots/review.png)
...
| 侧栏 | 顶栏 **☰** 或 **[** / **]** 开关；对局设置 / 棋谱；底部一排 **复盘 · 练习 · 每日**（今日到期数是徽标），下面是存档 · 统计 |
| 初见 | 第一次启动时棋盘下沿一句话：点棋盘就开始一局，或「先做今天的 5 道题」；落子后不再出现 |
| 设置 | 顶栏**齿轮**：主题 / 坐标 / 复盘分析 / 语言 / 音效（v1.51 起从侧栏搬进这里） |
| 人机 | 难度 简/普/难/**极**，每档是一句面向玩家的承诺并有闸门证伪（v1.63）：**入门** 挡住你的冲四但常漏活三（实测挡活三 19/40）· **普通** 必挡直接威胁、会算连续冲四（VCF 35/35），每手约 0.4 秒 · **困难** 看得懂四三、双三，会做杀会防杀，每手约 2 秒 · **极限** 同一引擎给足时间（难局 5–8 秒）并会算活三连杀；普/难/极 = **C3 引擎**（v1.65，三档对 v1.64 同档 24:0 / 24:0 / 23:1），难与极多出一行「思考」快/标/深 |
| 提示 | **H** / 顶栏「提示」：虚线十字建议点（不自动落子；**复盘中也可用**） |
| 全屏 | macOS：**View → Enter Full Screen（⌘⌃F）**，绿键 Zoom；Windows：双击标题栏或最大化按钮 |
| 棋谱 | 手数列表点击跳转；复制 / 导出 / 导入 / **粘贴** SGF（可拖入 `.sgf`） |
| 存档 | 侧栏底部「**存档**」：命名保存当前对局、历史列表、读取 / 重命名 / 删除；另有全部数据的备份 / 恢复 |
| 复盘 | 侧栏底部「**复盘**」打开侧栏复盘面板（v1.64 起唯一的复盘面，弹层退役）：**评分曲线**（零线两侧标着黑优 / 白优）、失着手数、当前这一手的「当时的威胁 → 你的落点 → 对手怎样惩罚 → 可行替代」，以及 **重下这一手 / 练这一手 / 推演 / 导出**。失着分三档证据——**可证明**（错失一步成五 / 漏防）· **引擎比较**（同一局面同一预算，引擎首选不同且分差可见）· **局势波动**（只有静态分差，**默认折叠**，不计入头上的数、不进终局卡）；第二遍在后台逐条问引擎，引擎也这么走的失着会被撤掉。导出带评注 SGF 时更优点与重下分支写成**变着** |
| 终局卡 | 一局下完，侧栏顶部出现总结卡（不弹层、棋盘可见）：一句结果、**本局值得记住的一手**（只从可证明与引擎比较里挑）、其他关键手；主行动「**重下第 N 手**」，次行动「再来一局」。侧栏收着时（窄于 900 的窗口不自动展开），棋盘下沿有一扇门 |
...
src/web/
  ...
  js/session.js    # 偏好与这一局分开 + 会话 kind 策略表（v1.64）
