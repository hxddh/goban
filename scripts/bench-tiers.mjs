 ...
 * | 旋钮 | 实测 | 判定 |
 * |---|---|---|
 * | 加深 maxDepth 14→24 | **安静局面** 30 个 0 处改变(中位仍烧满 5001ms) | 死 |
 ...
 * 结论没变,但前两个解释都是错的:
 *
 * - v1.57 说「搜索到 maxDepth 就返回,多给的算力花不出去」—— 错
 * - v1.58 说「搜索已经收敛、树被剪完了」—— 也错
 * - **实情**:安静局面上**时间先耗尽**,搜索根本走不到第 14 层,所以深度上限是
 *   一个**空的约束**。抬到 24 什么也不会变,不是因为它算完了,是因为它没算到那儿。
 *
 * 限制这台引擎的是**时间**;而在给定时间里唯一能换来棋力的是**宽度**。
 *
 * ## ⚠ 搜索类实验必须只在安静局面上做
 *
 * 这是这一轮踩得最贵的坑。引擎自战产生的局面里,**58% 是被战术级联
 * (forcedMove / findVCF)直接拍板的** —— 那些手根本不进搜索,任何搜索参数
 * 都改不了它们。在未过滤的局面集上做搜索实验,一多半样本是死的。
 *
 * 后果是双重的:
 *
 * 1. **v1.58 的加深实验(59 个未过滤局面)近乎空转**,结论侥幸对了,证据是虚的。
 *    只留安静局面重跑,才是上表那一行。
 * 2. **v1.58 报的「极档中位每手 10ms、根本没在算 5–8 秒」是误导性读数** ——
 *    那个 10ms 是被 58% 的级联手拉出来的。同一台引擎在**安静局面**上的中位是
 *    **5001ms**,它确实在烧满预算。v1.59 据此把极档文案从「5–8 秒深算」改成
 *    「根节点搜得更宽」,建立在这个误读上;v1.60 已改回带时间的准确说法。
 *
 * 过滤判据(便宜、够用):
 *
 ...
 *
 * findVCF 不给 ctx 就是无限预算,会把题面生成拖垮;给一个 eval 配额即可。
 *
